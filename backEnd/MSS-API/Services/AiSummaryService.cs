...
public class AiSummaryService : IAiService
{
    ...
    private async Task<ChannelSummaryResponse> GenerateSummaryAsync_Gemini(List<ProvideingVtuberData> targets)
    {
        ...
        // 2. URL の構築
        var url = $"https://generativelanguage.googleapis.com/v1beta/models/{ModelId}:generateContent?key={_settings.GeminiApiKey}";
        ...
        {
            ShowLog($"{response.StatusCode}エラー。 リクエスト失敗");
            return new();
        }
        ...
        var analyses = JsonSerializer.Deserialize<List<ChannelSummaryRaw>>(cleanedJson) ?? new();
